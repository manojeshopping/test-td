...
class Sns_I8style_Model_Import_Cms extends Mage_Core_Model_Abstract
{
	private $_importPath;
    ...
    {
        parent::__construct();
		$this->_importPath = Mage::getBaseDir() . '/app/code/local/Sns/I8style/etc/import/';
    }
	...
	public function importCms($modelString, $itemContainerNodeString, $overwrite = false)
    {
		try
		{
			...
			{
				throw new Exception(
					Mage::helper('i8style')->__("Can't get the data file for import cms blocks/pages: %s", $xmlPath)
                );
			}
			...
			if ($i)
			{
				Mage::getSingleton('adminhtml/session')->addSuccess(
					Mage::helper('i8style')->__('%s item(s) was(were) imported.', $i)
				);
			}
			else
			{
				Mage::getSingleton('adminhtml/session')->addNotice(
					Mage::helper('i8style')->__('No items were imported')
				);
			}
