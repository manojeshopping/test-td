 ...
 */ 
class Amasty_Shiprules_Adminhtml_RuleController extends Mage_Adminhtml_Controller_Action
{
    ...
    {
	    $this->loadLayout(); 
        $this->_setActiveMenu('sales/amshiprules/' . $this->_modelName . 's');
        $this->_addContent($this->getLayout()->createBlock('amshiprules/adminhtml_' . $this->_modelName)); 	    
 	    $this->renderLayout();
    }
    ...
    public function editAction() 
    {
		...
		$model  = Mage::getModel('amshiprules/' . $this->_modelName)->load($id);

		if ($id && !$model->getId()) {
    		Mage::getSingleton('adminhtml/session')->addError(Mage::helper('amshiprules')->__('Record does not exist'));
			$this->_redirect('*/*/');
			return;
		}
		...
		Mage::register('amshiprules_' . $this->_modelName, $model);
        ...
        $this->_addContent($this->getLayout()->createBlock('amshiprules/adminhtml_' . $this->_modelName . '_edit'));
        $this->_addLeft($this->getLayout()->createBlock('amshiprules/adminhtml_' . $this->_modelName . '_edit_tabs'));
	...
	public function saveAction() 
	{
	    ...
	    $model  = Mage::getModel('amshiprules/' . $this->_modelName);
		...
		if ($data) {
			...
			try {
				...
				$msg = Mage::helper('amshiprules')->__($this->_title . ' has been successfully saved');
        ...
        Mage::getSingleton('adminhtml/session')->addError(Mage::helper('amshiprules')->__('Unable to find a record to save'));
        $this->_redirect('*/*');
	} 
    ...
    public function massDeleteAction()
    {
        ...
        if (!is_array($ids)) {
             Mage::getSingleton('adminhtml/session')->addError(Mage::helper('amshiprules')->__('Please select records'));
             $this->_redirect('*/*/');
             return;
        }
    ...
    protected function _modifyStatus($status)
    {
        ...
        if ($ids && is_array($ids)){
            try {
                Mage::getModel('amshiprules/' . $this->_modelName)->massChangeStatus($ids, $status);
